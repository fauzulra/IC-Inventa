...
@section('content')
    <div class="max-w-7xl mx-auto bg-white p-6 rounded-lg shadow-md border border-gray-200 mt-6">
        ...
        @else
            <h2 class="text-xl font-semibold text-gray-800 mb-20">Daftar Material - {{ $project->name }}</h2>
        @endif
                ...
                <tbody id="tableBody" class="text-sm text-gray-600">
                    @forelse ($materials as $material)
                        <tr class="hover:bg-gray-50 transition">
                            ...
                            {{-- PERHATIKAN INI: Membaca stok dari tabel pivot --}}
                            <td class="border border-gray-200 px-4 py-1.5 ">
                                {{ $material->pivot->stock }}</td>
                            ...
                            <td class="border border-gray-200 px-4 py-1.5 text-center">
                                <div class="flex justify-center gap-2">
                                    {{-- Tombol Edit (Kirim pivot->stock, bukan stock global) --}}
                                    <button
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 border-b border-gray-100">
                    <div class="flex justify-between items-center">
                        <h3 class="text-lg leading-6 font-bold text-gray-900" id="modal-title">Tambah Material ke
                            {{ $project->name }}</h3>
                        <button onclick="toggleModal('modalTambahMaterial')"
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                    <form action="{{ route('material.store') }}" method="POST">
                        @csrf
                        {{-- WAJIB: Kirim ID Proyek secara tersembunyi agar controller tahu --}}
                        ...
                        <div class="flex gap-4 mb-4">
                            <div class="w-1/2">
                                ...
                                <input type="number" name="stock" value="0" readonly
                                    ...
                                    title="Kuantitas awal selalu 0. Tambah stok melalui menu Barang Masuk.">
                            </div>
                            <div class="w-1/2">
                        ...
                        <div class="flex gap-4 mb-4">
                            <div class="w-1/2">
                                ...
                                <input type="number" id="edit_stock" name="stock" readonly
                                    class="bg-gray-100 shadow cursor-not-allowed appearance-none border rounded w-full py-2 px-3 text-gray-500 leading-tight focus:outline-none focus:ring-2 focus:ring-[#FFB22C] focus:border-transparent">
                            </div>
                            <div class="w-1/2">
                                <label class="block text-gray-700 text-sm font-bold mb-2">Satuan</label>
    ...
    <div id="modalDeleteMaterial" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title"
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6">
                    <p class="text-sm text-gray-500 mb-4">Apakah Anda yakin ingin menghapus data material ini dari proyek
                        <b>{{ $project->name }}</b>?
                    </p>
...
@section('script')
    <script>
        ...
        // Fungsi Edit - Menerima nilai stock dari tabel pivot
        function openEditModal(id, name, stock, unit, supplierId) {
            ...
            document.getElementById('edit_stock').value = stock; // Ini sekarang adalah pivot stock
